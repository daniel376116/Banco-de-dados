...
--   - data do aluguel.
	...
	select c.id_cliente, nome_cliente, data_aluguel
	from clientes as c
...
--   - data do aluguel.
	...
	select c.id_cliente, nome_cliente, titulo, data_aluguel
	from clientes as c
	inner join alugueis as a
	on c.id_cliente = a.id_cliente
...
--   - data do aluguel.
	...
	select nome_cliente, titulo, data_aluguel
	from clientes as c
	inner join alugueis as a
	on c.id_cliente = a.id_cliente
	inner join filmes as f
...
--   - nome do ator.
	 ...
	 select att.id_ator, nome_ator, titulo, f.id_filme
     from atuacoes as a
	inner join filmes as f
     on a.id_filme = f.id_filme
     inner join atores as att
     on a.id_ator = att.id_ator
     order by nome_ator;
...
--   WHERE
	...
	select nome_cliente, titulo, genero, data_aluguel
    from clientes as c
    inner join alugueis as a
    on c.id_cliente = a.id_cliente
    inner join filmes as f
...
--   WHERE
	...
	select titulo, nome_ator, nacionalidade
    from filmes as f
    inner join atuacoes as a
    on f.id_filme = a.id_filme
    inner join atores as att
    on a.id_ator = att.id_ator
